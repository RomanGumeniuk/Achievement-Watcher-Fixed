  ...
  try {
    ...
    if (finalList.length > 0) {
      ...
      const promises = finalList.map(async (appid, index) => {
        ...
        endTime = Date.now();
        if (!game) {
          // FIX (fork): do NOT auto-blacklist on a failed load.
          // The upstream behaviour permanently added the appid to cfg/exclusion.db
          // the first time its schema failed to load -- which also happens for purely
          // transient reasons (no Steam Web API key set yet, a network hiccup, a rate
          // limit, the schema CDN being down). Once blacklisted the game was silently
          // skipped on every future scan, even after the key/network was fixed, so it
          // never reappeared. We now just skip it for this scan; it is retried on the
          // next one. Genuinely unwanted appids can still be removed via the hardcoded
          // bogus-list, the server list, or the "blacklist" action in the UI.
